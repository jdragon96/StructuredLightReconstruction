...
sl::ColorCodeInfo loadColorCodeInfo(const std::string& path) {
    cv::FileStorage fs(path, cv::FileStorage::READ);
    if (!fs.isOpened()) throw std::runtime_error("color_pattern_info.json을 열 수 없습니다: " + path);
    ...
    for (size_t i = 0; i < sequenceRaw.size(); ++i) sequence[i] = static_cast<int>(sequenceRaw[i]);
...
sl::PhaseInfo loadPhaseInfo(const std::string& path) {
    cv::FileStorage fs(path, cv::FileStorage::READ);
    if (!fs.isOpened()) throw std::runtime_error("color_phase_info.json을 열 수 없습니다: " + path);
