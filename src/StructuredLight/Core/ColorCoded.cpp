...
ColorCodeInfo ColorCodeInfo::create(const std::vector<int>& logicalSequence,
    ...
    if (selfEqualizing) {
        ...
        for (const auto& color : uniqueColors) info.palette.push_back(color.cast<float>() / 255.0f);
        info.decodeWindow = 2 * decodeWindow;
    } else {
        info.sequence = logicalSequence;
        info.palette.reserve(logicalPalette255.size());
        for (const auto& color : logicalPalette255) info.palette.push_back(color.cast<float>() / 255.0f);
...
ColorCodedReconstruction::ColorCodedReconstruction(ColorCodedConfig config, CameraCalibration camera,
    ...
    if (phaseInfo_.frequency <= 0) throw std::invalid_argument("RGB phase frequency must be positive");
}
...
int ColorCodedReconstruction::countAnchorWindows(const std::vector<int>& segmentLabels) const {
    ...
    while (i < filtered.size()) {
        size_t j = i;
        while (j < filtered.size() && filtered[j] == filtered[i]) ++j;
    ...
    for (int start = 0; start < std::max(0, last); ++start) {
        ...
        if (colorInfo_.windowToStart.count(key) > 0) ++count;
    }
    return count;
}
...
std::vector<int16_t> ColorCodedReconstruction::decodeSegment(const std::vector<float>& fractions,
    ...
    for (int state = 0; state < nStates; ++state) backtrack[0][static_cast<size_t>(state)] = static_cast<int16_t>(state);
    ...
    for (int x = 1; x < length; ++x) {
        ...
        for (int delta = -maxJump; delta <= maxJump; ++delta) {
            for (int current = 0; current < nStates; ++current) {
                ...
                if (previous < 0 || previous >= nStates) continue;
...
ColorCodedReconstruction::FringeOrderResult ColorCodedReconstruction::decodeFringeOrder(
    ...
    for (Eigen::Index y = 0; y < rows; ++y) {
        ...
        while (x < cols) {
            ...
            while (x < cols && phaseValid(y, x)) ++x;
            const Eigen::Index stop  = x;
            const int segmentLength  = static_cast<int>(stop - start);
            if (segmentLength < config_.minSegmentPixels) continue;
            ...
            if (countAnchorWindows(segmentLabels) < config_.minAnchorWindows) continue;
            ...
            for (int k = 0; k < segmentLength; ++k) orders(y, start + k) = segmentOrders[static_cast<size_t>(k)];
        }
    }
